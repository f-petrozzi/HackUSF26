...
import { getStoredUser, logout as apiLogout, refreshSessionUser } from "@/lib/api";
...
function ClerkAuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    refreshSessionUser(fallbackUser.full_name)
      .then((nextUser) => {
        if (!cancelled) setResolvedUser(nextUser);
      })
      .catch(() => {
        if (!cancelled) setResolvedUser(fallbackUser);
      })
      .finally(() => {
